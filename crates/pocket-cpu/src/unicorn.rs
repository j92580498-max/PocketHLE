...
/// Per-slice watchdog. The default is deliberately short enough for
/// the UI to receive input even if the guest is inside a render loop,
/// while ordinary API-heavy titles still stop on their regular thunks.
fn slice_watchdog_us(configured_us: u64) -> u64 {
    if configured_us != 0 {
    ...
    fn run_until_hook(
        ...
        *self.stop_requested.borrow_mut() = false;
        ...
        let r = self.uc.emu_start(
            ...
            slice_watchdog_us(self.slice_timeout_us), // timeout (us)
