...
GAMES = [
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 4, "result": "Gameplay rendered; PCM audio captured",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Boots through GLES to the title screen",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Continuous rendering and audio submission verified",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Gameplay scene rendered; audio path verified",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Non-black gameplay scene rendered",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Loading and language menu rendered",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Title screen and gameplay capture rendered",
    ...
    },
    {
        ...
        "cpu": "ARM / Unicorn", "rating": 3, "result": "Rendered gameplay surface captured",
...
def app_table(prefix: str) -> str:
    rows = "".join(
        f'''<tr><td><a href="{relative(prefix, f"apps/{game['slug']}/index.html")}">{esc(game['name'])}</a></td><td>{esc(game['target'])}</td><td>{stars(game['rating'])}</td><td>{esc(game['result'])}</td></tr>'''
        for game in GAMES
    )
    return f'''<h3>List</h3><table class=searchable-table><thead><tr><th>App name</th><th>Target</th><th>Best rating</th><th>Result</th></tr></thead><tbody>{rows}</tbody></table>'''
...
<h3>Reports</h3>
<table><thead><tr><th>CPU</th><th>Rating</th><th>Remarks</th><th>Proof</th></tr></thead><tbody>
<tr><td>{esc(game['cpu'])}</td><td>{stars(game['rating'])}</td><td>{esc(game['proof'])}</td><td><a href="{game['proof_url']}">View</a></td></tr>
</tbody></table>
